...
document.addEventListener('DOMContentLoaded', function () {
    ...
    searchInputs.forEach(function (input) {
        input.addEventListener('change', function () {
            loadPosts(1); // Gọi hàm loadPosts() để tải danh sách dịch vụ
        });
    });
    ...
    paginationButtons.forEach(function (button) {
        button.addEventListener('click', function () {
            if (!this.classList.contains('active')) {
                ...
                loadPosts(page); // Gọi hàm loadPosts() để tải danh sách dịch vụ của trang được chọn
            }
        });
    });
    ...
    function loadPosts(page = 1) {
        ...
        xhr.open('GET', 'post?event=post-list-userchoose&postTitle=' + encodeURIComponent(postTitle) +
