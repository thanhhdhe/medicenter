...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function handleInputChange(page = 1) {
        ...
        xhr.open('GET', 'medical-examination?event=patient-of-staff&patientName=' + encodeURIComponent(patientName)
                +'&page=' + encodeURIComponent(page));

        xhr.onload = function () {
            if (xhr.status === 200) {
                // Xử lý dữ liệu trả về từ máy chủ và cập nhật nội dung trang
                document.querySelector('#children-list').innerHTML = xhr.responseText;
